        ...
        for place in filtered_data:
            filtered_place = {
                ...
                "total_ratings": place.get(
                    "user_ratings_total", "Total ratings not available"
                ),
                "business_status": place.get(
                    "business_status", "Business status not available"
                ),
                "opening_hours": place.get("opening_hours", {}).get(
                    "open_now", "Opening hours not available"
                ),
                "types": place.get("types", "Types not available"),
                "photo_reference": place.get("photos", [{}])[0].get(
                    "photo_reference", "Photo reference not available"
                ),
